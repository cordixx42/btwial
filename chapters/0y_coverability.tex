...
For this Petri net, the corresponding transducer needs to accept the word \break ${(\bm{1 \square 11 \square 111 \square},\bm{1 \square 11111 \square 111 \square})}$. However, the length of these words $w$ and $v$ is not the same, but the transducer can only recognize a pair of words with equal length by definition. 
...
%\begin{algorithm}[htb]
%\caption{Construction of $\mathcal{T}$ given a transition $t \in \Tr$ }\label{alg:trans}
%\begin{algorithmic}[1]
%\Input {$t = (g,h)$}
%\State $c \gets 0$
%\ForEach { $g_{i} \in g$ }
%\State $\delta = h_{i} - g_{i}$
%\State $d \gets \min (g_{i},h_{i})$
...
%        \node[
		...
		\node[place,
			fill=blue!25,
    		draw=blue!75,
   			label=below:$s_3$] (s3) at (3,0.5) {};

		\node[transition, label=above:$t_1$] (t1) at (3,3) {}  
		
%		edge [pre and post] (s1)
		\draw   (s1) edge[above, bend right] node{} (t1)
		\draw   (t1) edge[left, bend right] node{} (s1) 	
		\draw   (s2) edge[above, bend right] node{$3$} (t1)
		\draw   (t1) edge[below, bend right] node{$2$} (s2)
		\draw   (s3) edge[above, bend right] node{} (t1)
...
%\begin{algorithm}[htb]
%\caption{Preprocess}\label{alg:prep}
%\begin{algorithmic}[1]
%\Input {$t = (g,h), q \in \tbl, j \in \mathbb{N} $}
%\Procedure{preprocess}{$t,q,j$}
%\If {$\cache[q]$}
%\Return {$\cache[q]$}
%\EndIf
%\State $\cache[q] \gets \self$
%\State $b \gets \final(q)$
%\State $s \gets [q_{\emptyset},\dots,q_{\emptyset}]$
%%\ForEach {$a_{i} \in \Sigma$}
%\If {$\Succ(q,\square) \neq q_{\emptyset} \land (h_{j}-g_{j}) > 0 $}
%\State add $h_{j}-g_{j}$ new nodes containing only $\Diamond$ transitions between $q$ and $\Succ(q,\square)$
%\State 
%\Else
%\State $s_{\bm{1}} \gets \Call{preprocess}{t,\Succ(q,\bm{1}),j}$
%\If{$\Succ(q,\square) \neq q_{\emptyset}$}
%\State $j \gets j + 1$
%\EndIf
%\State $s_{\square} \gets \Call{preprocess}{t,\Succ(q,\square),j}$
%\EndIf
%\State $q' \gets T.\make(s,b)$
%\State $\cache[S] \gets q'$
%\Return $q'$
%\EndProcedure
%\end{algorithmic}
%\end{algorithm}
