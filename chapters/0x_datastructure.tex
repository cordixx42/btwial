...
\begin{definition}[\cite{blondin_24}]\label{def:master-aut}
...
We define the operations $\Succ$ and $\final$ to access node elements by the node identifier for future algorithms.
...
This is the main change in the next $\make$ procedure in \autoref{alg:makev1}. The first part with the lookup for an existing node stays the same as in \autoref{alg:make1}. But if the node does not exist yet, we add all possible combinations, denoted by $\pi(s)$, in lines 5 and 6. For the mapping from identifier to $s$ and $b$, the successor array without $\self$s is used. This is performed by line 7, where $s_{[\self / q_{\text{new}}]}$ denotes the array $s$, with the only difference that each $\self$ is substituted by $q_{\text{new}}$. 
\par
\begin{algorithm}[htb]
\centering
...
\begin{definition}[\cite{blondin_24}]\label{def:pre}
...
\begin{lemma}[\cite{blondin_24}]\label{lem:b}
...
The structure of $B$ shows similarities with the product construction for the intersection of two DFAs. In fact, $B$ can be viewed precisely as the product of the intersection of the DFA $A$ with the image of the transducer $\mathcal{T}$. 

\autoref{fig:pre-result} shows $B$ for the example in \autoref{fig:pre}.
Each state in $B$ is a pair of a state in $\mathcal{T}$ and a state in $A$. The initial state $q_{0B}$ is the pair of initial states of $\mathcal{T}$ and $A$, in \autoref{fig:pre-result} this is the pair $(q_{1},q_{3})$. The final states are the pairs of final states of both $\mathcal{T}$ and $A$. In \autoref{fig:pre-result} the state $(q_{2},q_{4})$ is final, as $q_{2}$ and $q_{4}$ are also final in the transducer and the DFA.
...
The transition function $\delta_{B}$ takes a state $(p,q)$, where $p$ is a state of the transducer, $q$ a state of the DFA and a letter $x$. The set of states given back are all $(p',q')$, where there is a letter $y$ with the following conditions:
\begin{itemize}[--,noitemsep]
\item In the transitions of $\mathcal{T}$ there exists the transition $p \xrightarrow{(x,y)} p'$, and
\item in the transitions of $A$ there exists the transition $q \xrightarrow{y} q'$.
\end{itemize}

Taking \autoref{fig:pre-result} as an example, there exists a transition from $(p,q)=(q_{1},q_{3})$ to $(p',q')=(q_{2},q_{4})$ with letter $x=\bm{a}$ in $B$, because there is $y=\bm{c}$ such that:

\begin{itemize}[--,noitemsep]
\item In the transitions of $\mathcal{T}$ there exists the transition $q_{1} \xrightarrow{(\bm{a},\bm{c})} q_{2}$ (\autoref{fig:pre-transducer}), and
\item In the transitions of $A$ there exists the transition $q_{3} \xrightarrow{\bm{c}} q_{4}$ (\autoref{fig:pre-dfa}).
\end{itemize}
Using this construction, the complete NFA $B$ in \autoref{fig:pre-result} characterizes the language $L(\bm{b^{*}a})$ as expected.
