...
Let $\widehat{C}$ denote the unique minimal automaton of $C$. Each state in $\widehat{C}$ recognizes a distinct language and $L(\widehat{C}) = L(C)$. Let $L_{M}(p)$ denote the language recognized from state $p$ in an automaton $M$.

Assume for contradiction $L(C) \neq L(\widetilde{C})$. This means, in line 3 of \autoref{alg:proof} non-equivalent states have been collapsed.
...
As $p,q$ form a cycle in $C$, there exist non-empty words $v,w \in \Sigma^{*}$ such that $\delta_{C}(p,v) = q$ and $\delta_{C}(q,w) = p$. 
