...
The overall memory measurements show good results for \textit{ptm} in the easier instances, but they are an enormous amount higher than the other two tools in numerous instances like \instance{extended-read-write-sc} or \instance{pncsacover}.
...
Only the plot of the \instance{bingham\_25} instance behaves very differently to the others. The size of $q$ slowly increases here with more iterations and in the end there is even a small leap before the algorithm terminates. 
